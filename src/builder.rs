...
[Cost {}] ({} cards)", cost, sum_in_cost);
            ...
            for (card, count) in sorted_group {
                ...
                println!("  {}x {} ({})", count, card.name, type_label);
            }
        }
    }
}
...
impl DeckBuilder {
    pub fn build<'a>(legend: &'a Card, champion: &'a Card, scored: &[ScoredCard<'a>]) -> Decklist<'a> {
        ...
        for s in scored {
            // exclude legend and starting champion from being added to the main deck copies again in this simplified builder
            if s.card.name == legend.name || s.card.name == champion.name {
                continue;
            }

            // Only Units, Spells, and Gear go in the Main Deck. Legends/Runes/Battlefields do not.
        ...
        let mut fill_bucket = |bucket_key: u32, target: usize, deck_cards: &mut Vec<(&'a Card, usize)>, total_added: &mut usize| {
        ...
        fill_bucket(2, 8, &mut deck_cards, &mut total_added);
        
        // 2. Fill the rest of the curve
        fill_bucket(0, 3, &mut deck_cards, &mut total_added);
        fill_bucket(1, 4, &mut deck_cards, &mut total_added); 
        fill_bucket(3, 9, &mut deck_cards, &mut total_added);
        fill_bucket(4, 7, &mut deck_cards, &mut total_added);
        fill_bucket(5, 8, &mut deck_cards, &mut total_added);

        // 3. If we haven't reached 39 cards yet (e.g., missing specific cost slots), 
        // backfill with the highest synergy cards remaining regardless of curve.
